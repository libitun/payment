...
async function main() {
  ...
  try {
    ...
    console.log("  ⚠ Registry:", err.message ? err.message.slice(0, 160) : err);
  }
  ...
  try {
    ...
    console.log("  ⚠ Treasury:", err.message ? err.message.slice(0, 160) : err);
  }
  ...
  const outPath = path.join(__dirname, 'mints.json');
